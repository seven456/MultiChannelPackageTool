...
//		args = "-path D:/111_m360.zip -password 12345678".split(" ");
		...
		String help = "用法：java -jar MCPTool.jar [-path] [arg] [-contents] [arg] [-password] [arg]"
				+ "\n" + cmdPath + "		APK文件路径"
				+ "\n" + cmdOutdir + "		输出路径（可选），默认输出到APK文件同一目录"
				+ "\n" + cmdContents + "	写入内容集合，多个内容之间用“;”分割，如：googleplay;m360; 当没有" + cmdContents + "”参数时输出已有文件中的content"
				+ "\n" + cmdPassword + "	加密密钥（可选），产度8位以上，如果没有该参数，不加密"
				+ "\n" + cmdVersion + "	显示版本号"
				+ "\n例如："
